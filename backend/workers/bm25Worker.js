...
function rankByKeywordsBM25(publications, query) {
  ...
  publications.forEach((p) => { p.keywordScore = p.keywordScore / maxScore; });

  return publications;
}

try {
  const { publications, query } = workerData;
  const scored = rankByKeywordsBM25(publications, query);
  parentPort.postMessage({ publications: scored });
} catch (err) {
